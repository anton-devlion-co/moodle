...
class core_course_renderer extends plugin_renderer_base {
    ...
    public function course_section_cm($course, &$completioninfo, cm_info $mod, $sectionreturn, $displayoptions = array()) {
        ...
        $data = [];
        $data['indentclasses'] = $indentclasses;
        if ($this->page->user_is_editing()) {
            $data['cmmove'] = course_get_cm_move($mod, $sectionreturn);
        }
        ...
        if ($this->page->user_is_editing()) {
            ...
            $data['modicons'] .= ' ' . $this->course_section_cm_edit_actions($editactions, $mod, $displayoptions);
            $data['modicons'] .= $mod->afterediticons;
        }
        $data['modicons'] .= $this->course_section_cm_completion($course, $completioninfo, $mod, $displayoptions);
        // Show availability info (if module is not available).
        $data['cmavailability'] = $this->course_section_cm_availability($mod, $displayoptions);
        if (!empty($url)) {
            $data['contentparturl'] = $contentpart;
        }
        return $this->output->render_from_template('core_course/activity_raw', $data);
    }
